...
void Player::UsingToolState::Enter(Player &player)
{
    ...
    ml::vec2 playerPos = player.GetPosition();
    if (playerPos.x < 0)
        playerPos.x -= SPRITE_SIZE;
    if (playerPos.y < 0)
        playerPos.y -= SPRITE_SIZE;
    ml::vec2 actionCoords = ml::vec2(((int)playerPos.x / (int)SPRITE_SIZE + player.direction.x) * SPRITE_SIZE, ((int)playerPos.y / (int)SPRITE_SIZE + player.direction.y) * SPRITE_SIZE) + ml::vec2(SPRITE_SIZE, SPRITE_SIZE) / 2;
